...
pub fn tokenize(allocator: std.mem.Allocator, source: []const u8) ![]Token {
    ...
    var receiver_pending = false;
    var receiver_depth: u32 = 0;
    ...
    while (i < source.len) {
        ...
        if (isDigit(c)) {
            ...
            while (i < source.len) {
                const d = source[i];
                if (isDigit(d) or d == '_' or d == '.' or d == 'e' or d == 'E' or d == 'p' or d == 'P' or d == '+' or d == '-' or d == 'i') {
                    i += 1;
                    col += 1;
                    continue;
                }
        ...
        if (isAsciiIdentStart(c)) {
            ...
            if (isKeyword(word)) {
                ...
                if (std.mem.eql(u8, word, "func")) {
                    const next = peekNonWhitespace(source, i);
                ...
                } else if (std.mem.eql(u8, word, "var") or std.mem.eql(u8, word, "const")) {
        ...
        if (receiver_pending) {
            if (c == '(') {
                receiver_depth += 1;
            } else if (c == ')' and receiver_depth > 0) {
                receiver_depth -= 1;
                if (receiver_depth == 0) {
